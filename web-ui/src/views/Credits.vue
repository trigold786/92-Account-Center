<template>
  <div>
    <el-row :gutter="16">
      <el-col :span="12"><el-card><template #header>积分余额</template><p style="font-size:48px;color:#6C63FF">{{ account?.balance || '--' }}</p></el-card></el-col>
      <el-col :span="12"><el-card><template #header>累计</template><p>获得: {{ account?.total_earned || 0 }}</p><p>消耗: {{ account?.total_consumed || 0 }}</p></el-card></el-col>
    </el-row>
