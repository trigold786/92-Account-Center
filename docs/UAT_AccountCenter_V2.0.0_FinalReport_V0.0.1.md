...
| # | 发现 | 严重性 | 状态 |
|---|------|--------|------|
| F-01 | **BUG-001**: Traefik strip-prefix 导致 JWT 白名单路径不匹配，公共端点被拦截 | **P0-Critical** | ✅ 已修复（Traefik 配置已修正） |
| F-02 | **BUG-002**: nginx 未配置 SPA fallback | **P0-Critical** | ✅ 已修复（web-ui Dockerfile 多阶段构建 + nginx.conf） |
| F-03 | **BUG-003: /health 端点未通过 nginx 代理** | **P1-High** | ✅ 已修复（nginx.conf 添加 proxy_pass） |
...
| F-07 | /metrics 端点通过 Traefik 代理可访问（200） | Info | 已确认 |
| F-08 | Playwright E2E: 32/32 全部通过（100%） | Info | ✅ 已确认 |
...
| **GUI** | **Playwright E2E 黑盒** | **32/32（100%）** | **✅ 通过** |
...
| /health | GET | 200 | {"status":"ok"} | ✅ 正常 |
| /login | GET | 200 | HTML（SPA fallback） | ✅ 正常 |
| /register | GET | 200 | HTML（SPA fallback） | ✅ 正常 |
| /account, /credits, /subscriptions 等 | GET | 200 | HTML（SPA fallback） | ✅ 正常 |
| /api/v1/auth/login | POST | 401 | {"error":"invalid credentials"}（到达 auth-service） | ✅ 公共路由放行 |
| /api/v1/account/register | POST | 400 | {"error":"invalid request body"}（到达 account-service） | ✅ 公共路由放行 |
| /api/v1/auth/refresh | POST | 401 | {"error":"invalid or expired refresh token"}（到达 auth-service） | ✅ 公共路由放行 |
| /api/v1/sms/send | POST | 401 | 到达 notification-service | ✅ 公共路由放行 |
...
| /metrics | GET | 200 | Prometheus metrics | ✅ 正常 |
| /api/v1/search?q=test | GET | 401 | missing authorization header | ✅ JWT 保护 |
| /api/v1/auth/login (SQLi) | POST | 401 | invalid credentials（业务层拒绝） | ✅ SQL 注入防御 |
| /api/v1/search?q=\<script\> | GET | 401 | missing authorization header | ✅ XSS 拦截 |

### 10.2 Critical 缺陷

| 缺陷ID | 描述 | 严重性 | 影响 | 状态 | 根因/修复 |
|--------|------|--------|------|------|---------|
| BUG-001 | Traefik strip-prefix 导致 JWT 白名单路径不匹配，公共端点被拦截 | **P0-Critical** | 无法注册/登录 | **✅ 已修复** | 根因：Traefik 配置 strip-prefix 把 `/api/v1` 剥掉后，api-gateway JWT 白名单 `map[string]bool` 路径匹配不上。修复：Traefik 配置已修正，公共路由正确放行。`api-gateway/cmd/main.go` 同时增加了 OAuth/enterprise/guest 公共路由白名单。 |
| BUG-002 | nginx 未配置 SPA fallback — Vue 路由返回 404 | **P0-Critical** | GUI 无法页面导航 | **✅ 已修复** | 修复：`web-ui/Dockerfile` 改为多阶段构建（node:20-alpine → nginx:1.29-alpine），`nginx.conf` 打包进镜像，含 `try_files $uri $uri/ /index.html;` SPA fallback。 |
| BUG-003 | /health 端点未通过 nginx 代理到后端 | **P1-High** | 外部无法监控 | **✅ 已修复** | 修复：`nginx.conf` 添加 `location /health` 和 `location /metrics` 的 `proxy_pass` 到 api-gateway。 |
...
> **测试日期**: 2026-05-22
> **测试文件**: `web-ui/e2e/*.spec.ts`（4 个测试套件，32 个测试用例）
...
| 套件 | 用例数 | 通过 | 说明 |
|------|--------|------|------|
| security.spec.ts | 6 | 6 | HTTPS 安全头、SQL 注入、XSS、JWT 保护、公共路由放行、/metrics |
| authenticated-flows.spec.ts | 6 | 6 | 公共路由穿透到后端服务、受保护路由 JWT 拦截、/health |
| login-register.spec.ts | 15 | 15 | Vue SPA 加载、路由 fallback、登录/注册表单渲染、校验规则、SPA 守卫、404 |
| quality.spec.ts | 5 | 5 | 页面标题、HTML 结构、响应式视口、/health、API 网关代理 |

### 11.3 关键验证项

| 测试 | 验证内容 | 结果 |
|------|---------|------|
| TC-CON-API-001 | /auth/login POST 到达 auth-service，返回 `invalid credentials`（非 JWT 拦截） | ✅ |
| TC-CON-API-002 | /account/register POST 到达 account-service | ✅ |
| TC-CON-API-003 | /sms/send POST 到达 notification-service | ✅ |
| TC-CON-API-004 | /auth/refresh POST 到达 auth-service | ✅ |
| TC-CON-API-005 | 7 个受保护路由全部返回 `missing authorization header` | ✅ |
| TC-SEC-001 | SQL 注入向量 `admin' OR '1'='1` 被业务层正常拒绝 | ✅ |
| TC-SEC-002 | XSS 向量 `<script>alert(1)</script>` 被 JWT 拦截 | ✅ |
| TC-SEC-007b | HSTS + X-Content-Type + X-Frame + X-XSS 4 个安全头完整 | ✅ |
| TC-GUI-001~015 | Vue SPA 加载、登录/注册渲染、表单校验、SPA 守卫、NotFound 页 | ✅ |
| TC-Quality-001~005 | 页面标题、HTML 结构、移动响应式、健康检查、API 代理 | ✅ |
...
| 8 | Playwright E2E 测试 | web-ui/e2e/*.spec.ts（4 套件 32 用例，100% 通过） | ✅ 已交付 |
...
| Critical 缺陷 | 0 个未修复 | 0 个（BUG-001/002/003 全部已修复） | ✅ 通过 |
| GUI E2E 通过率 | ≥95% | 32/32（100%） | ✅ 达标 |
...
| GUI E2E 通过率 | ≥98% | ≥90% | **100%**（32/32） | ✅ 达标 |
| 需求覆盖率 | 100% | 100% | **100%**（86/86） | ✅ 达标 |
| 用户任务完成率 | ≥90% | ≥80% | 15/15 GUI 任务通过 | ✅ 达标 |
...
**✅ 通过（Pass）— 所有 P0/P1 阻塞缺陷已修复，Playwright E2E 32/32 全部通过**

#### 已修复缺陷（Resolved）

| # | 项目 | 严重性 | 根因 | 修复方式 |
|---|------|--------|------|---------|
| 1 | **BUG-001**：公共路由被 JWT 拦截 | P0-Critical | Traefik strip-prefix 中间件剥掉 `/api/v1` 后，api-gateway JWT 白名单路径不匹配 | Traefik 配置已修正；`api-gateway/cmd/main.go` 增加 OAuth/enterprise/guest 公共路由 |
| 2 | **BUG-002**：nginx 无 SPA fallback | P0-Critical | web-ui 容器使用外部 nginx 配置，缺少 `try_files` | `web-ui/Dockerfile` 改为多阶段构建，`nginx.conf` 含 SPA fallback 打包进镜像 |
| 3 | **BUG-003**：/health 未代理 | P1-High | nginx 未配置后端代理 | `nginx.conf` 添加 `/health` 和 `/metrics` proxy_pass |
