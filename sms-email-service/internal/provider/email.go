...
func NewSMTPProvider(host, port, username, password, from string) EmailProvider {
	return &smtpProvider{
		host:     host,
		port:     port,
		username: username,
...
func (p *smtpProvider) SendVerificationCode(ctx context.Context, to, code string) error {
	addr := p.host + ":" + p.port
	subject := "验证码 - 账户中心"
	body := fmt.Sprintf(
